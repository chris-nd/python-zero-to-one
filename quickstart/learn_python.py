...
b = a  # b pointe sur a
print(b is a)  # => True, a et b pointent sur le même objet
print(b == a)  # => True, les objets a et b sont égaux
b = [1, 2, 3, 4]  # b pointe sur une nouvelle liste, [1, 2, 3, 4]
print(b is a)  # => False, a et b ne pointent pas sur le même objet
print(b == a)  # => True, les objets a et b ont la même valeur

# Les chaînes (ou strings) sont créées avec " ou '
print("Ceci est une chaine")
print("Ceci est une chaine aussi.")
...
print("{} peuvent etre {}".format("Les chaînes", "interpolées"))
...
print("{name} wants to eat {food}".format(
...
name = "Fred"
print(f"Il a dit que son nom est {name}.")  # => "Il a dit que son nom est Fred."

# Si votre code doit aussi être compatible avec Python 2.5 et moins,
# vous pouvez encore utiliser l'ancienne syntaxe :
print("Les %s peuvent être %s avec la %s méthode" % ("chaînes", "interpolées", "vieille"))
...
try:
    print(une_variable_inconnue)  # Lève une NameError
...
try:
    print(li[4])  # Lève une IndexError
...
try:
    ...
    valid_dict = {
        (1, 2, 3): [1, 2, 3]
    }  # Par contre, les valeurs peuvent être de tout type.
except TypeError as error:
    print(error)
...
some_set = {1, 2, 3, 4}  # some_set est maintenant {1, 2, 3, 4}
...
with open("get-started-quickly/myfile.txt") as f:
    for line in f:
        print(line)
...
print(
    our_iterable
)  # => range(1,10). C'est un objet qui implémente l'interface Iterable
...
try:
    print(our_iterable[1])  # Lève une TypeError
except TypeError as error:
    print(error)
...
# Une compréhension de liste stocke la sortie comme une liste qui peut
# elle même être une liste imbriquée.
...
for item in double_numbers(range_):
    print(item)
    if item >= 30:
        break
